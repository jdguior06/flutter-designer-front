...
import { RotateCcw, RotateCw, Trash2, Eye, Code, MessageSquare, Moon, Sun, Save, Cloud, CheckCircle } from "lucide-react";
...
export default function DesignerWorkspace({ projectId }: DesignerWorkspaceProps) {
  ...
  useEffect(() => {
    ...
    const handleElementsUpdated = (data: any) => {
      const { projectId: updatedProjectId, screenId, elements, timestamp } = data;
      
      // Solo actualizar si es el mismo proyecto pero diferente usuario
      if (updatedProjectId === projectId && screenId === currentScreenId) {
        console.log('🔄 Elementos actualizados por otro usuario:', elements.length);
 ...
 const removeElement = useCallback(
  ...
  const renameScreen = useCallback(async (id: string, name: string) => {
    try {
      ...
      setScreens((prevScreens) => 
        prevScreens.map((screen) => (screen.id === id ? { ...screen, name } : screen))
      );
    } catch (error) {
  ...
  const addScreenIA = useCallback(async (name: string, elements: Omit<DesignElement, 'id'>[] = []) => {
    if (!currentProject || !user) return;
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <div className="flex h-12 items-center justify-between border-b border-gray-200 bg-white px-4">
          <div className="flex items-center space-x-2">
            ...
            >
              {isSaving ? <Cloud size={20} className="animate-pulse" /> : <Save size={20} />}
            </button>

            <span className="text-sm text-gray-500">
              Elements: {currentScreen ? currentScreen.elements.length : 0}
              {hasUnsavedChanges && <span className="text-orange-500 ml-2">• Sin guardar</span>}
